...
/**
 * Script to update document chunks with meaningful content using mock embeddings
 * This avoids API quota issues while providing searchable content
 */
...
async function updateWithMockEmbeddings() {
  try {
    console.log('🔄 Starting content update with mock embeddings...\n');
    ...
    for (const file of allFiles) {
      try {
        ...
        // Generate mock embeddings for the chunks
        const chunkInserts = chunks.map((chunk, index) => ({
          fileId: file.id,
          content: chunk.content,
          chunkIndex: chunk.chunkIndex,
          metadata: chunk.metadata,
          embedding: JSON.stringify(generateMockEmbedding(chunk.content))
        }));
    ...
    if (updatedCount > 0) {
      ...
      console.log('📝 Note: Using mock embeddings for development. In production, use real embedding services.');
    }
