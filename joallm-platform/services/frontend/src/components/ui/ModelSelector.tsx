...
import { ChevronDown, Zap, Clock, DollarSign, Star, Home, Cloud, Lock } from 'lucide-react';
import { useLLM } from '../../contexts/LLMContext';
import { useUserRole } from '../../contexts/EnhancedUserRoleContext';

const PRO_ONLY_PROVIDERS = ['OpenAI', 'Anthropic'];

function isModelLockedForTier(provider: string, modelId: string, tier: string): boolean {
  if (tier !== 'free') return false;
  if (!PRO_ONLY_PROVIDERS.includes(provider)) return false;
  // gpt-3.5 is allowed on free
  if (modelId.toLowerCase().startsWith('gpt-3.5')) return false;
  return true;
}
        ...
        {isOpen && (
          <div className="absolute top-full right-0 mt-2 w-80 bg-white shadow-xl border border-gray-200 rounded-lg overflow-hidden">
            <div className="p-3 border-b border-gray-100 bg-gray-50">
              <h3 className="font-semibold text-gray-900">ATRISI Marketing</h3>
              <p className="text-xs text-gray-600 mt-1">Choose from multiple AI models for your task</p>
            </div>
            
            <div className="max-h-96 overflow-y-auto">
              {availableModels.map((model) => {
                const locked = isModelLockedForTier(model.provider, model.id, subscriptionTier);
                return (
                  <button
                    key={model.id}
                    onClick={() => {
                    ...
                    disabled={locked}
                    title={locked ? 'Upgrade to Pro to unlock this model' : undefined}
                    ...
                    <div className="flex items-start justify-between mb-2">
                      <div className="flex-1">
                        <div className="flex items-center space-x-2 mb-1">
                          ...
                          {selectedModel.id === model.id && !locked && (
            ...
            {subscriptionTier === 'free' && (
              <div className="border-t border-gray-100 bg-amber-50 px-4 py-3 text-xs text-amber-800">
                ...
                GPT-4 and Claude models require Pro. Groq models are included on the free plan.
              </div>
            )}
          </div>
        )}
