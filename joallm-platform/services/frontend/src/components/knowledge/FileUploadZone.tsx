...
import { Upload, File, Image, FileText, X, AlertCircle, CheckCircle, Loader2, Lock, ChevronDown, ChevronUp } from 'lucide-react';
import { AttachmentStatus } from '../../types';
import { showError } from '../../utils/toast';
import { getSupportedFileExtensions, FILE_CONFIG } from '../../utils/fileValidation';
...
export function FileUploadZone({
  ...
  onFileRemove,
  maxFileSize = 50,
  acceptedTypes, // Will default to supported formats if not provided
  multiple = true,
  showPreviews = true,
}: FileUploadZoneProps) {
  ...
  // Use centralized supported formats if not provided
  const finalAcceptedTypes = acceptedTypes || getSupportedFileExtensions(true).split(',');
  ...
  const validateFile = (file: File): string | null => {
    ...
    if (!finalAcceptedTypes.some(type => type === fileExtension || type === '*')) {
      return `File type not supported. Accepted: ${finalAcceptedTypes.join(', ')}`;
    }
        ...
        <div className="flex flex-col items-center gap-3">
          ...
          <div>
            ...
            <p className="text-xs text-gray-500">
              Supported: TXT, MD, DOC, DOCX, PDF (Beta), Images, Code files • Max {maxFileSize}MB
            </p>
          </div>
        </div>
      ...
      <div className="mt-4 border-t border-gray-200 pt-4">
        ...
        >
          <div className="flex items-center gap-2">
            ...
            <span className="text-sm font-medium text-gray-700">Coming Soon Formats</span>
            <span className="text-xs text-gray-500 bg-gray-100 px-2 py-0.5 rounded-full">
              In Development
            </span>
          </div>
          {showComingSoon ? <ChevronUp className="w-4 h-4 text-gray-400" /> : <ChevronDown className="w-4 h-4 text-gray-400" />}
        </button>

        {showComingSoon && (
          <div className="mt-3 p-4 bg-gray-50 rounded-lg">
            <p className="text-xs text-gray-600 mb-3">
              These file formats are actively being developed and will be available soon:
            </p>
            
            <div className="grid grid-cols-2 gap-2">
              {FILE_CONFIG.COMING_SOON_FORMATS.map(format => (
                <div
                  key={format}
                  className="flex items-center gap-2 p-2 bg-white border border-gray-200 rounded text-xs text-gray-600"
                  title="This format is in development"
                >
              ...
              {FILE_CONFIG.ARCHIVE_FORMATS.map(format => (
                <div
                  key={format}
                  className="flex items-center gap-2 p-2 bg-white border border-gray-200 rounded text-xs text-gray-600"
              ...
              {FILE_CONFIG.EBOOK_FORMATS.map(format => (
                <div
                  key={format}
                  className="flex items-center gap-2 p-2 bg-white border border-gray-200 rounded text-xs text-gray-600"
                  title="Ebook support coming soon"
                >
                  <Lock className="w-3 h-3 text-gray-400" />
                  <span>{format.toUpperCase()}</span>
                </div>
            ...
            <p className="mt-3 text-xs text-gray-500 italic">
              💡 Tip: For now, try converting these files to .docx or .txt for best results.
            </p>
          </div>
        )}
      </div>
