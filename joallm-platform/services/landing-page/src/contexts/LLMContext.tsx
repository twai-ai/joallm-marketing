...
export function LLMProvider({ children }: { children: ReactNode }) {
  ...
  const [selectedModel, setSelectedModel] = useState<LLMModel>(defaultModels[0]);
    ...
    const fetchModels = async () => {
      ...
      try {
        ...
        if (models.length > 0) {
          ...
          // Set the first model as selected if no model is currently selected
          if (!selectedModel || selectedModel.id === defaultModels[0].id) {
            setSelectedModel(models[0]);
          }
        }
      } catch (error) {
        console.error('❌ Failed to fetch models from API, using default models:', error);
        console.error('API URL:', `${API_BASE_URL}/api/models`);
  ...
  useEffect(() => {
    if (availableModels.length > 0 && selectedModel) {
      // Check if the currently selected model still exists in the new list
      const modelExists = availableModels.find(model => model.id === selectedModel.id);
      if (!modelExists) {
        // If the selected model no longer exists, select the first available model
