...
export const storage = {
  ...
  // Clear only secure/encrypted data
  clearSecure(): void {
    try {
  ...
  getSecure<T>(key: string, defaultValue?: T): T | null {
    try {
      ...
      // Check if encrypted data is empty or invalid
      ...
      // Check if decrypted data is empty or invalid
...
export const setRefreshToken = (token: string): void => {
  storage.setSecure(STORAGE_KEYS.REFRESH_TOKEN, token);
};
