...
export function AcquisitionIntelligencePage() {
  ...
  const canManageIntegrations =
    ...
    tenant.role === 'admin' ||
    tenant.role === 'owner';
  const metaAuthenticated = Boolean(
  ...
  return (
    <UseCaseHomeShell
      ...
      description="Institution-owned Acquisition — shared people and Meta channels for your organization. Platform Identity handles admission and permissions."
      primaryAction={
        <>
          <button
            type="button"
          ...
          >
            {syncingInsights ? <Loader2 className="h-4 w-4 animate-spin" /> : <Activity className="h-4 w-4" />}
            Sync insights
          </button>
      ...
      secondaryPanelTitle="Institution → Timeline"
      secondaryPanelBody="Organization owns people and campaigns. Your membership role gates actions. Events land on the shared Person Timeline."
      secondaryPanelContent={
        <div className="grid grid-cols-2 gap-3 text-sm">
          {[
            ['People', overview?.people],
            ['Events', overview?.events],
            ['Org', tenant?.organizationCode || '—'],
            ['Role', tenant?.role || '—'],
          ].map(([label, value]) => (
            <div
              key={String(label)}
    ...
    >
      <section className="mb-6 rounded-3xl border border-slate-200 bg-white p-5 shadow-sm">
        <div className="flex flex-wrap items-center justify-between gap-3">
          <div>
            <h2 className="text-lg font-semibold text-slate-950">Meta status</h2>
            <p className="mt-0.5 text-sm text-slate-600">
              {metaAuthenticated
                ? 'Auth healthy — waiting on inbound traffic and leads where steps are still open.'
                : 'Bind channels and confirm Graph health. Setup details are below.'}
            </p>
          </div>
          {tenant && (
            <span className="rounded-full bg-slate-100 px-3 py-1 text-xs font-semibold text-slate-700 ring-1 ring-slate-200">
              {tenant.organizationCode} · {tenant.role}
            </span>
          )}
        </div>
        <div className="mt-4 grid gap-3 sm:grid-cols-3">
          {[
            {
              label: 'WhatsApp',
              ok: Boolean(health?.graphOk && health?.boundToUser),
              detail: health?.displayPhoneNumber || health?.verifiedName || 'Not live',
            },
            {
              label: 'Page / IG',
              ok: Boolean(pageHealth?.graphOk && pageHealth?.boundToUser),
              detail: pageHealth?.pageName || pageHealth?.igUsername || 'Not live',
            },
            {
              label: 'Ads / Leads',
              ok: Boolean(marketingHealth?.graphOk && marketingHealth?.boundToUser),
              detail: marketingHealth?.adAccountName || `${marketingHealth?.leadsIngested ?? 0} leads`,
            },
          ].map((chip) => (
            <div
              key={chip.label}
            ...
            >
              <div className="flex items-center justify-between gap-2">
                <span className="text-sm font-semibold text-slate-950">{chip.label}</span>
                <span
                  className={`rounded-full px-2 py-0.5 text-[10px] font-semibold uppercase ${
                    chip.ok ? 'bg-teal-100 text-teal-800' : 'bg-slate-200 text-slate-600'
                  }`}
                >
                  {chip.ok ? 'Live' : 'Setup'}
                </span>
              </div>
              <p className="mt-1 truncate text-xs text-slate-600">{chip.detail}</p>
            </div>
          ))}
        </div>
      </section>
